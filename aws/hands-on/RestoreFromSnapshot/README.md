...
```
  ...
  - Delete `employees` who earns salary above `$70000` from the `clarusway` db on `root-RDS`.
    ...
    ```
### STEP 2 - Restore from manual Snapshot
...
- Restore database from manual snapshot as new DB instance and name it as `restored-from-man-snapshot`.
  ...
  - Instance configuration
    DB instance class: Burstable classes/db.t3.micro (select include previous generation classes)
...
### STEP 3 - Connect to RDS created from manual Snapshot
...
- Delete the manual snapshot !!!!
  -  Go to left hand menu >>> Select Snapshots >>>>> manual Snapshot  >>> delete 
...
- Note: Double check and make sure there doesn't exist any manual or system(automated) snapshots after deleting all the three database instances
