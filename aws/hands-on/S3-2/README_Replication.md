...
- Go to `source.replica.clarusway` bucket and direct upload `non_tag_file.txt`.

- Go to `destination.cross.region.replica.clarusway` bucket and show that the `non_tag_file.txt` is not replicated in the bucket.
